...
import { getUserListings } from '@/utils/FireBase';
  ...
  return (
    <div>
        ...
        listingData&& listingData.map((item,index)=>(
            ...
            key={index}
          >
            <h2 className='bg-primary text-white absolute px-2 text-sm p-1 m-1 rounded-lg'>{item.active?'Published':'Draft'}</h2>
            <Image
            ...
            <div className="flex mt-2 gap-2 flex-col">
              ...
              <div className='flex gap-2'>
                <Link href={'/view-listing/'+item.id} className='w-full'><Button size='sm' className='w-full' variant='outline'>View</Button></Link>
                <Link href={'/edit-listing/'+item.id} className='w-full'><Button size='sm' className='w-full'>Edit</Button></Link>
